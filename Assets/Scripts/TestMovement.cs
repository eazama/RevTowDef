...
public class TestMovement : MonoBehaviour
{
	...
	void Update()
	{
		if(!moving && path.Count > 0) {
			...
			StartCoroutine(Move(gameObject.transform.position, new Vector3(next.x,
			                                                               next.y,
	...
	public void getPath()
	{
		GridCoord start = new GridCoord(Mathf.RoundToInt(gameObject.transform.position.x),
		                                Mathf.RoundToInt(gameObject.transform.position.y));
		GameObject target = GameObject.FindGameObjectWithTag("Goal");
		GridCoord finish = new GridCoord(Mathf.RoundToInt(target.transform.position.x),
